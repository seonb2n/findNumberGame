...
try {
	...
	//서버에 score 전송
	String sql_insert = "";
	sql_insert = "INSERT INTO "+TABLE_NAME+ " VALUES ( '"+userNAME+"', "+userSCORE+")";
	resultSet = stmt.executeQuery(sql_insert);
	
	
	//sql 문 내의 데이터를 5개 가져기.
	sql_insert = "SELECT * FROM (SELECT * FROM " +TABLE_NAME+ " ORDER BY "+COL_SCORE+" DESC) WHERE ROWNUM<="+rankNumber;
	resultSet = stmt.executeQuery(sql_insert);
